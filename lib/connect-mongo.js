...
module.exports = function(connect) {
  ...
  function MongoStore(options, callback) {
    ...
    if ( !options.mongoose ) {
      throw new Error('mongoose option is mandatory');
    }
    
    try {
      this.sessionModel = mongoode.model(this._mongooseModelName);
    } catch(e) {
      this._setupSchema(options.mongoose);
    }
    ...
    if (options.mongoose_connection){
      ...
      if (Array.isArray(options.mongoose_connection.hosts)) {
        ...
        this.db = new mongo.Db(options.mongoose_connection.db.databaseName, set, {w: options.w || defaultOptions.w });
      } else {
  ...
  MongoStore.prototype._mongooseModelName = 'Session';
  
  MongoStore.prototype._setupSchema = function(mongoose) {
    var schema = new mongoose.Schema({
      _id: {type: String},
      session: {type: mongoose.Schema.Types.Mixed},
      expire: {type: Date, index : { expires : 0 }}
    });
    this.sessionModel = mongoose.model(this._mongooseModelName, schema);
  };
  ...
  MongoStore.prototype.get = function(sid, callback) {
    var self = this;
    this.sessionModel.findOne({_id: sid}, function(err, session) {
      if (err) {
  ...
  MongoStore.prototype.set = function(sid, session, callback) {
    try {
      var s = {session: this._serialize_session(session)};
      ...
      this.sessionModel.update({_id: sid}, s, {upsert: true, safe: true}, function(err) {
        if (err) {
          callback && callback(err);
        } else {
  ...
  MongoStore.prototype.destroy = function(sid, callback) {
    this.sessionModel.remove({_id: id}, function() {
      callback && callback();
    });
    /*
    this._get_collection(function(collection) {
  ...
  MongoStore.prototype.length = function(callback) {
    this.sessionModel.count({}, function(err, count) {
      if (err) {
        callback && callback(err);
      } else {
  ...
  MongoStore.prototype.clear = function(callback) {
    this.sessionModel.remove(function() {
      callback && callback();
    });
    /*
    this._get_collection(function(collection) {
